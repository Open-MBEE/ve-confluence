...
import type { UrlString, SparqlString } from '../common/types';
...
export class SparqlDataConnection implements DataConnection {
	...
	static fromSerialized(g_serialized: SerializedSparqlDataConnection) {
		return new SparqlDataConnection(
			g_serialized.endpoint,
			g_serialized.modelGraph,
			g_serialized.metadataGraph,
		);
	}

	constructor(
		p_endpoint: UrlString,
		p_model_graph: UrlString,
		p_metadata_graph: UrlString | string | null = null,
	) {
		...
		// derive from model graph
			...
			const s_project = /\/rdf\/graph\/(Model|Project)\.()(?:\.|$)/.exec(
				this._p_model_graph,
			);
			this._p_metadata_graph =
				du_model_graph.origin + `/rdf/graph/Metadata.${s_project}`;
		}
	}
